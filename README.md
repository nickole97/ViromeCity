...
├── CONTRIBUTING.md           ← how to edit the book
├── _quarto.yml               ← navigation, theme, resources
├── index.qmd                 ← the cover
├── sections/                 ← the chapters (.qmd)
├── assets/
│   ├── styles.scss           ← the only stylesheet
│   └── img/                  ← Fivi + figures
├── kitchen/                  ← "Fivi's Kitchen" wizard (hand-written HTML)
├── components/               ← body heatmap + richness curve (hand-written HTML)
├── _archive/sections-html/   ← the legacy pages, kept until migrated
└── docs/                     ← BUILD OUTPUT — never edit by hand
```

`docs/` is generated by `quarto render` and served by GitHub Pages from
`main` / `/docs`. Anything you write there is erased on the next build.
...
[Install Quarto](https://quarto.org/docs/get-started/), then run `quarto preview`
from the repo root — it opens the book and reloads as you edit. See
